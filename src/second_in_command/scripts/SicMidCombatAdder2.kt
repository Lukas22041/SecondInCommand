...
class SicMidCombatAdder2 : FleetMemberDeploymentListener{
    ...
    init{
        ...
        for (a in engine.ships) {
            ...
            //log?.info("NOT Setting 'SicMidCombatAdder' perset data of ship, force, owner: "+a.id+", "+a.originalOwner+", "+a.fleetCommander.id)
        }
        Global.getCombatEngine().customData.put("SiC_SCDataMap",map);
        Global.getCombatEngine().customData.put("SiC_SCDataMap_2",map2);
    }
    override fun reportFleetMemberDeployed(member: DeployedFleetMemberAPI?) {
        ...
        if (a != null && a.hullSize == ShipAPI.HullSize.FIGHTER){
