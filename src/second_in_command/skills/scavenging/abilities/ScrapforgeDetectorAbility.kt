...
class ScrapforgeDetectorAbility : BaseDurationAbility() {
    ...
    override fun activateImpl() {
        ...
        var deployable = system.addCustomEntity("sc_scrapforge_deployable_${Misc.genUID()}", "Detector Relay", "sc_scrapforge_deployable", fleet.faction.id)
        deployable.location.set(Vector2f(loc))
        var plugin = deployable.customPlugin as ScrapforgeDeployable
        plugin.type = ScrapforgeDeployable.Type.DETECTOR
        plugin.timeToLife = 30f
    }
    ...
    fun addAbilityTooltip(tooltip: TooltipMakerAPI, spec: AbilitySpecAPI) {
        ...
        tooltip.addSpacer(10f)
        tooltip.addPara("Deploys a small detector relay that lasts for 30 days. It periodically emits sensor bursts that scan the entire system to reveal points of interest. " +
